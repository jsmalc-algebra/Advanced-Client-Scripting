...
async function itemizeItems(item_data,bill_data){
    ...
    for (let item of item_data) {
        ...
        itemizedItems.push(
            new Item(item_data,bill_data,product_data,subcategory_data,category_data)
        )
...
export async function getItems(page,limit,sortBy,sortOrder,searchString,billId) {
    ...
    if(sortBy === null || !LOCAL_SORT_FIELDS.includes(sortBy)){
        ...
        ({data:item_data,totalCount} = await fetchPaginatedItemDataByBillId(page,limit,sortBy,sortOrder,searchString));
    } else {
